...
We are asking you to build a (greenfield) CRUD application. The backend is to be created in Java using Spring Boot; we have provided a little bit of boilerplate. The communication between this application and other applications is to be done via a RESTFul API that you will design and build (bonus points if you provide an Open API (a.k.a. Swagger) Specification). In addition to the backend, please build a (simple) API client that consumes the API.

Try to time-box your effort to 3 hours. We ask you to return the outcome as a .zip file. Your deliverable should be a good representation of how you work on a daily basis and therefore the assignment is no more than a framework in which you can display your ability. We are interested in argumentation behind the choices and solutions you make. This can mean that you will not deliver a completely working application, due to the time constraint.
